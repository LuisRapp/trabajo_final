...
<div class="min-h-screen !bg-slate-50 px-4 sm:px-6 lg:px-8 py-6">
    <div class="mb-6 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        ...
        <p class="text-xs !text-slate-500">Rango actual: <span class="font-semibold !text-slate-700"><?php echo e($rango_label ?? ''); ?></span></p>
    </div>
    ...
    <?php else: ?>
        ...
        <div id="reporteModal"
            ...
            <div style="width:100%; max-width:640px; background:#fff; border:1px solid #e2e8f0; border-radius:12px; padding:20px; box-shadow:0 20px 40px rgba(15,23,42,0.25);">
                ...
                <form id="reporteForm" method="GET" action="<?php echo e(route('reportes.estadisticas-forestales.pdf')); ?>" target="_blank" style="display:grid; gap:16px; margin-top:16px;">
                            ...
                            <div>
                                ...
                                <datalist id="lotesList">
                                    ...
                                    <?php $__currentLoopData = $lotes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $lote): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <option value="Lote #<?php echo e($lote->id_lote); ?> - <?php echo e($lote->ubicacion ?? $lote->propietario ?? 'Sin nombre'); ?>" data-id="<?php echo e($lote->id_lote); ?>"></option>
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4 mb-6">
            <div class="rounded-lg !bg-white p-4 shadow-sm">
                ...
                <div class="mt-2 text-lg font-semibold !text-slate-900">$<?php echo e(number_format($estadisticas_globales['precio_promedio'], 2)); ?>/tn</div>
            </div>
            <div class="rounded-lg !bg-white p-4 shadow-sm">
                <p class="text-xs font-medium !text-slate-500">Costo Promedio</p>
                <div class="mt-2 text-lg font-semibold !text-slate-900">$<?php echo e(number_format($estadisticas_globales['costo_promedio'], 2)); ?>/tn</div>
            </div>
            <div class="rounded-lg !bg-white p-4 shadow-sm">
                <p class="text-xs font-medium !text-slate-500">Punto de Equilibrio</p>
                <div class="mt-2 text-lg font-semibold !text-slate-900">$<?php echo e(number_format($estadisticas_globales['punto_equilibrio'], 2)); ?>/tn</div>
            </div>
            <div class="rounded-lg !bg-white p-4 shadow-sm">
                <p class="text-xs font-medium !text-slate-500">Rentabilidad Promedio</p>
                <div class="mt-2 text-lg font-semibold <?php echo e($estadisticas_globales['rentabilidad_promedio'] >= 0 ? '!text-emerald-600' : '!text-rose-600'); ?>">
            ...
            <div class="rounded-lg !bg-white shadow-sm">
                <div class="border-b !border-slate-200 px-4 py-3">
                    <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                        <h2 class="text-sm font-semibold !text-slate-900">Producción vs Punto de Equilibrio (<?php echo e($rango_label ?? ''); ?>)</h2>
                        <form method="GET" action="<?php echo e(route('reportes.estadisticas-forestales')); ?>" class="flex flex-nowrap items-center gap-2">
                            <input type="date" name="desde" value="<?php echo e($filtro_desde ?? ''); ?>" max="<?php echo e(now()->toDateString()); ?>"
                                class="h-8 w-32 min-w-[8rem] rounded-md border !border-slate-300 px-2 text-xs !text-slate-900" />
                            <span class="text-xs !text-slate-400">—</span>
                            <input type="date" name="hasta" value="<?php echo e($filtro_hasta ?? ''); ?>" max="<?php echo e(now()->toDateString()); ?>"
                                class="h-8 w-32 min-w-[8rem] rounded-md border !border-slate-300 px-2 text-xs !text-slate-900" />
        ...
        <div class="grid grid-cols-1 gap-4 mb-6 lg:grid-cols-2">
            ...
            <div class="rounded-lg !bg-white shadow-sm">
                <div class="border-b !border-slate-200 px-4 py-3">
                    <h2 class="text-sm font-semibold !text-slate-900">Evolución Costo por Tonelada (<?php echo e($rango_label ?? ''); ?>)</h2>
                </div>
                <div class="p-4">
                            ...
                            <?php $__currentLoopData = $lotes_estadisticas; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $stat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <tr>
                                    <td class="px-4 py-3 text-sm font-semibold !text-slate-900 break-words">
                                        <?php echo e($stat['nombre'] ?? 'Sin nombre'); ?>

                                    </td>
                                    <td class="px-4 py-3 text-sm !text-slate-700"><?php echo e(number_format($stat['hectareas'], 2)); ?> ha</td>
                                    ...
                                    <td class="px-4 py-3 text-sm">
                                        <span class="inline-flex items-center rounded-full <?php echo e($stat['rentabilidad'] >= 0 ? '!bg-emerald-100 !text-emerald-700' : '!bg-rose-100 !text-rose-700'); ?> px-2.5 py-1 text-xs font-semibold">
        ...
        <div class="mt-6 rounded-lg border !border-slate-200 !bg-white p-4 text-sm !text-slate-700 shadow-sm">
            <span class="font-semibold !text-slate-900">Resumen:</span> Analizando <?php echo e($estadisticas_globales['total_lotes']); ?> lote(s) activo(s) 
            con rentabilidad promedio de <span class="font-semibold !text-slate-900">$<?php echo e(number_format($estadisticas_globales['rentabilidad_promedio'], 2)); ?>/tn</span>.
        </div>
    <?php endif; ?>
</div>
...
<script>
    ...
    const optionsProduccion = {
        ...
        xaxis: {
            ...
            title: { text: 'Fechas (<?php echo e($rango_label ?? ""); ?>)' }
        },
        yaxis: {
    ...
    const optionsEvolucion = {
        ...
        xaxis: {
            ...
            title: { text: 'Período (<?php echo e($rango_label ?? ""); ?>)' }
        },
        yaxis: {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/rluis/Escritorio/trabajo_final/rennova/resources/views/reportes/estadisticas-forestales.blade.php ENDPATH**/ ?>