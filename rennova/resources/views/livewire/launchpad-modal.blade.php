    ...
    <div x-show="open" x-transition class="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
        <div class="w-full max-w-6xl rounded-2xl bg-white shadow-2xl ring-1 ring-black/5">
            <div class="flex items-center justify-between border-b px-6 py-4">
                <div>
                    <h2 class="text-xl font-semibold">Launchpad · Lote #{{ $loteId }}</h2>
            ...
            <div class="grid grid-cols-1 gap-6 p-6 md:grid-cols-3">
                <div class="rounded-xl border p-4">
                    <div class="mb-3 flex items-center justify-between">
                        ...
                        <span class="text-xs text-slate-500">Seleccionados: {{ collect($employeeSelected)->filter()->count() }}</span>
                    </div>
                    <div class="space-y-3">
                ...
                <div class="rounded-xl border p-4">
                    ...
                    <div class="space-y-3">
                        @forelse($machinery ?? collect() as $row)
                            @php
                ...
                <div class="rounded-xl border p-4">
                    <div class="mb-3 flex items-center justify-between">
                        ...
                        <div class="text-xs text-slate-500">
                            Fuel: <span class="font-semibold">{{ number_format($week_1_fuel, 2, ',', '.') }}</span>
                        </div>
                    </div>
                    <div class="space-y-3">
                    ...
                    <div class="mt-4 text-right text-sm">
                        Total semana 1: <span class="font-semibold">${{ number_format($supplies_cost, 0, ',', '.') }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center justify-between gap-3 border-t px-6 py-4">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button type="button" wire:click="confirmAndLaunch" @if($guardando) disabled @endif
