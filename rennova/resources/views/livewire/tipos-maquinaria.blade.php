                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse ($tipos as $tipo)
                                <tr wire:key="row-{{ $tipo->id_tipo_maquinaria }}" class="hover:bg-slate-50 transition-colors">
                                    <td class="px-4 py-2.5"><span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-slate-100 text-slate-600">{{ $tipo->id_tipo_maquinaria }}</span></td>
                                    <td class="px-4 py-2.5 font-medium text-slate-800">{{ $tipo->nombre }}</td>
                                    <td class="px-4 py-2.5 text-right">
                                        <x-action-buttons
                                            editWireClick="editar({{ $tipo->id_tipo_maquinaria }})"
                                            deleteWireClick="eliminar({{ $tipo->id_tipo_maquinaria }})"
                                            deleteMessage="¿Está seguro de eliminar este tipo?"
                                            :canEdit="auth()->user()->can('editar-tipos-maquinaria')"
                                            :canDelete="auth()->user()->can('eliminar-tipos-maquinaria')" />
                                    </td>
                                </tr>
                            @empty
