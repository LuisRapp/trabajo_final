<div class="w-full px-4 py-6 sm:px-6 lg:px-8">
    ...
    <div class="mb-6 flex gap-0">
        ...
        <button type="button" wire:click="$set('tab_activo','listado')"
            class="inline-flex items-center gap-2 px-4 py-3 font-semibold text-sm border rounded-r-sm transition-all {{ $tab_activo === 'listado' ? 'bg-pino text-white border-pino' : 'bg-white text-tinta-suave border-arena hover:bg-corteza-suave' }} {{ !auth()->user()->canAny(['crear-tipos-maquinaria', 'editar-tipos-maquinaria']) ? 'rounded-l-sm' : '' }}">
                    ...
                    <table class="data-table">
                        ...
                        <tbody>
                            @forelse ($tipos as $tipo)
                                <tr wire:key="row-{{ $tipo->id_tipo_maquinaria }}">
                                    <td><x-ui.badge variant="neutral">{{ $tipo->id_tipo_maquinaria }}</x-ui.badge></td>
                                    <td class="font-medium text-tinta">{{ $tipo->nombre }}</td>
                                    <td class="text-right">
                                        <div class="flex gap-1 justify-end">
                                            @can('editar-tipos-maquinaria')
                                                <x-ui.button size="sm" variant="secondary" icon="pencil-square" wire:click="editar({{ $tipo->id_tipo_maquinaria }})" title="Editar" />
                                            @endcan
                                            @can('eliminar-tipos-maquinaria')
                                                <x-ui.button size="sm" variant="danger" icon="trash" wire:click="eliminar({{ $tipo->id_tipo_maquinaria }})" wire:confirm="¿Esta seguro de eliminar este tipo?" title="Eliminar" />
                                            @endcan
                                        </div>
                                    </td>
                                </tr>
