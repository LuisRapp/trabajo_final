...
<body>
        ...
        <table>
            <tr>
                ...
                <td style="width: 55%; text-align: right;">
                    ...
                    <div class="subtitle">Recibo #{{ $recibo->id_recibo }}</div>
                    <div class="subtitle">Fecha emisión: {{ \Carbon\Carbon::parse($recibo->fecha_emision)->format('d/m/Y H:i') }}</div>
                    <div class="subtitle">Generado por: {{ $generado_por ?? 'Usuario' }}</div>
    ...
    <table>
        ...
        <tbody>
            <tr>
                <td>ARS {{ number_format($recibo->monto_bruto, 2) }}</td>
                <td>ARS {{ number_format($recibo->descuentos ?? 0, 2) }}</td>
                <td><strong>ARS {{ number_format($recibo->monto, 2) }}</strong></td>
            </tr>
        </tbody>
    </table>
        ...
        <tbody>
            <tr>
                <td class="kpi" width="33%">
                    ...
                    <div class="kpi-value">ARS {{ number_format($recibo->monto_bruto, 2) }}</div>
                </td>
                <td class="kpi" width="33%">
                    <div class="kpi-label">Descuentos</div>
                    <div class="kpi-value">ARS {{ number_format($recibo->descuentos ?? 0, 2) }}</div>
                </td>
                <td class="kpi" width="33%">
                    <div class="kpi-label">Neto</div>
                    <div class="kpi-value">ARS {{ number_format($recibo->monto, 2) }}</div>
                </td>
            </tr>
        </tbody>
    </table>
    ...
    <footer>
        Documento generado automáticamente. Rennova © {{ date('Y') }}
    </footer>
</body>
</html>
