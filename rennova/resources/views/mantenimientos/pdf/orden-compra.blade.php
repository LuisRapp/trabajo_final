...
<body>
        ...
        <table>
            <tr>
                ...
                <td style="width:80%;text-align:right;vertical-align:top;">
                    ...
                    <div class="doc-subtitle">Mantenimiento #{{ $proposal->id_mantenimiento }}</div>
    ...
    <main>
        ...
        <table class="table-items">
            ...
            <tbody>
                @forelse($proposal->insumos as $item)
                    @php $um = $item->insumo->unidadMedida->abreviatura ?? ''; @endphp
                    <tr>
                        <td>{{ $item->insumo->nombre ?? 'Insumo' }}</td>
                        <td class="text-right">{{ number_format((float) $item->cantidad_requerida, 2) }} {{ $um }}</td>
                        <td class="text-right">{{ number_format((float) $item->stock_disponible, 2) }} {{ $um }}</td>
                        <td class="text-right">{{ number_format((float) $item->faltante, 2) }} {{ $um }}</td>
                    </tr>
                @empty
                    <tr>
